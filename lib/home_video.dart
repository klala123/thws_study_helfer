...
import 'Firebase/_insert.dart';
import 'app_theme.dart';
import 'package:flutter/material.dart';
import 'package:image_picker/image_picker.dart';
import 'package:flutter/cupertino.dart';
import 'model/videoHomeList.dart';
import 'package:smooth_page_indicator/smooth_page_indicator.dart';


class MyVideoHomePage extends StatefulWidget {
  const MyVideoHomePage({Key? key}) : super(key: key);

  @override
  _MyVideoHomePageState createState() => _MyVideoHomePageState();
}

//-------------------------------------------------------------------------------------
// Die klasse erhält die Fähigkeit, Ticker bereitzustellen, die von Animation verwendet werden
class _MyVideoHomePageState extends State<MyVideoHomePage> with TickerProviderStateMixin {
  List<videoHomeList> homeList = videoHomeList.homeList;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Column(
          children: [
            Padding(
              padding: const EdgeInsets.only(top: 60.0, left: 32.0, right: 40.0, bottom: 16.0),
              child: getTextField(),
            ),
            Expanded(
              child: StreamBuilder<List<videoHomeList>>(
                  stream: videoHomeList.getVideoHomeListStream(),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return Center(child: CircularProgressIndicator());
                    } else if (snapshot.hasData) {
                      List<videoHomeList>? homeList = snapshot.data;
                      ...
                      for (int i = 0; i < homeList!.length; i++) {
                        if (homeList[i]
                          ...
                          // groupKey: homeList[i].groupKey,
                            ...
                            onDelete: () {
                              // Add your logic to delete the group with the groupKey here
                      ...
                      for (int i = 0; i < pageCount; i++) {
                        ...
                        pages.add(Padding(
                          padding: const EdgeInsets.only(
                              top: 100, left: 30, right: 40, bottom: 40),
                          child: GridView(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => ccreate()),
            );
          },
